...
def main():
    ...
    print()
    print(f"{BOLD}AI Red Teaming Engine{R} {DIM}v0.6.0{R}")
    ...
    print(f"{BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{R}")
    print(f"{BOLD} v0.6.0 新機能{R}")
    print(f"{BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{R}")
    pause(0.3)

    print(f"\n  {CYAN}$ {R}", end="")
    typewriter("redteam-scan -f app.py --format html -o report.html", delay=0.045)
    pause(0.3)
    print(f"  {GREEN}✓{R}  ブラウザで開けるインタラクティブHTMLレポートを生成")
    pause(0.5)

    print(f"\n  {CYAN}$ {R}", end="")
    typewriter("redteam-scan --init-ci", delay=0.045)
    pause(0.3)
    print(f"  {GREEN}✓{R}  .github/workflows/redteam.yml を生成（GitHub Actions）")
    print(f"  {GREEN}✓{R}  .redteam-ignore を生成")
    pause(0.5)

    print(f"\n  {CYAN}$ {R}", end="")
    typewriter("redteam-scan -f agent.py --injection-test --injection-types benign_wrapped", delay=0.04)
    pause(0.3)
    print(f"  {GREEN}✓{R}  段階的境界浸食攻撃（benign包装）をシミュレーション")
    pause(0.3)
    print(f"  {DIM}  BW-001: 研究者フレーム → 監査モード要求{R}")
    print(f"  {DIM}  BW-002: 信頼蓄積 → 制約解除要求{R}")
    print(f"  {DIM}  BW-003: セキュリティ学習 → 攻撃コード要求{R}")
    print()
    pause(0.5)
    print(f"{GREEN}完了 {DIM}(4.1s){R}")
    print()
